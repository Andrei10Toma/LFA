...
class Lexer:
    ...
    def __init__(self, dfas: List[Dfa], word) -> None:
        self.dfas = dfas
        self.word = word
    ...
    def compute_lexemes(self) -> List[List[str]]:
        ...
        while self.current_word_position < len(self.word):
            ...
            if len(list(dfa_filtered)) == len(self.dfas):
                return self.current_word_position
...
def runlexer(lexer: str, finput: str, foutput: str) -> None:
    ...
    for i in range(0, len(readLines)):
        if readLines[i] == '\n':
            dfas.append(Dfa(readLines[start : i]))
            start = i + 1

    dfas.append(Dfa(readLines[start : len(readLines)]))
